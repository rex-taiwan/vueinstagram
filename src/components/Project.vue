    ...
    >
      ...
      <v-btn icon>
        <v-badge overlap color="red">
          ...
          <v-icon size="32" color="grey darken-2">
            mdi-menu
          </v-icon>
        </v-badge>
      </v-btn>
    ...
    <v-container class="drageffect mb-12" style="max-width:800px;">
      ...
      <v-row class="mt-10" v-if="loading">
        <v-col cols="4" v-for="number in imagegrid" :key="number">
          <v-skeleton-loader :loading="loading" v-show="loading" type="image">
            <v-card> </v-card>
          </v-skeleton-loader>
        </v-col>
      </v-row>

      <div v-if="!loading" class="text-center subtitle-2 grey--text">
        <span class="font-weight-bold black--text">28</span> profile visits in
        the last 7 days
      </div>
      <div
        class="mx-2"
        v-for="project in myProjects.slice(0, 1)"
        :key="project.title"
      >
          ...
          <v-row>
            ...
            <v-col cols="12" md="9">
              <p class="font-weight-bold title">
                {{ project.person }}
                <v-btn small text class="ml-2 success">
                  <v-icon size="18" class="mx-1">mdi-account-plus</v-icon>
                  Follow</v-btn
                >
              </p>
              <p>
                <v-icon small class="mx-1 black--text">mdi-map-marker</v-icon
                >Taiwan
                <v-icon small class="mx-1 black--text">mdi-earth</v-icon
                >https://rex-taiwan.github.io/vueinstagram/project
              </p>
              <p class=" subtitle-2">
                Lorem ipsum, dolor sit amet consectetur adipisicing elit.
                Aspernatur hic nulla harum mollitia voluptate aut aliquam. Et
                officiis, saepe dicta dolore esse suscipit laborum? Maxime saepe
                dolore laboriosam quam ipsa!
              </p>
            </v-col>
          </v-row>
      ...
      <v-row class="mt-8" no-gutters>
              ...
              <v-list-item>
                ...
                <v-menu offset-y>
                  ...
                  <v-list>
                    ...
                    <v-list-item>
                      <v-list-item-title>
                        <v-icon>mdi-share</v-icon> Report</v-list-item-title
                      >
                    </v-list-item>
                  </v-list>
                </v-menu>
        ...
        <v-icon>mdi-trash-can</v-icon>
            ...
            <div class="white d-none d-sm-block">
              <v-row no-gutters>
                <v-col cols="12" md="6">
                  <v-img style="height:800px;" :src="project.p" :class="project.filter">
                  </v-img>
                </v-col>
                <v-col cols="12" md="6">
                  <v-card flat>
          ...
          <v-icon>mdi-plus-circle</v-icon>
                    ...
                    <v-list-item>
                      ...
                      <v-menu offset-y>
                        ...
                        <v-list>
                          <v-list-item
                            @click.prevent="deleteProjects(project.id)"
                          >
                            <v-list-item-title>
                    ...
                    <p
                      class="subtitle-1 font-weight-bold black--text mx-4 mt-5 text-lowercase"
                    >
                      <v-avatar size="40" class="mr-2 my-2"
                        ><img :src="project.imgUrl"
                      /></v-avatar>
                      ...
                      <strong
                        class="d-block font-weight-regular mt-12"
                        style="color:#003569"
                        >#{{ project.title }}</strong
                      >
